...
class Doge(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        #the skin variable is what determines how the doge looks (what image gets loaded)
        global skin
        skin = "run1.png"
    ...
    def update(self):
        ...
        #trying to animate a gif + Dealing with orientation
        global Orient
        global blip
        blip += 1
        global image_changing_score
        if blip == 5:
            ...
            blip = 0
        if image_changing_score % 2 == 0 and Orient == 0:
            skin = "moving?.png"
        if image_changing_score % 2 == 0 and Orient == 1:
            skin = "flippedmoving?.png"
        if image_changing_score % 2 != 0 and Orient == 0:
            skin = "run1.png"
        if image_changing_score % 2 != 0 and Orient == 1:
            skin = "flippedrun1.png"
        self.image = pygame.image.load(skin).convert_alpha()
...
def game_over():
    ...
    while True:
        ...
        Font = pygame.font.SysFont("freesansbold", 100, bold = 1)
        number_of_stolen_doritos = str(score)
        TextSurface = Font.render(f"You stole {number_of_stolen_doritos} doritos!", 1, pygame.Color(255,255,255))
        T = TextSurface.get_rect()
